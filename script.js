...
const loadFile =(e)=>{
    ...
    if(!file){
        return; //return if user has not selected any file
    }
    previewImg.src = URL.createObjectURL(file);//=> passing selected file url to preview img src    
    //URL.createObjectURL creates a URL of the passed object.

    previewImg.addEventListener("load", ()=>{//=>once image is loaded
        widthInput.value = previewImg.naturalWidth; //=> returns original width of an image
...
const resizeAndDownload=()=>{
    ...
    a.href = canvas.toDataURL("image/jpeg", imgQuality); //=> toDataURL() returns a url of the image
